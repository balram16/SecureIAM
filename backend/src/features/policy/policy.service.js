 ...
 */
const validatePolicyStatements = (statements) => {
  if (!Array.isArray(statements)) {
    const error = new Error('Statements must be an array.');
    error.statusCode = 400;
    throw error;
  }

  if (statements.length === 0) {
    const error = new Error('Statements array cannot be empty.');
    error.statusCode = 400;
    throw error;
  }
  ...
  for (const statement of statements) {
    if (!statement || typeof statement !== 'object') {
      const error = new Error('Each statement must be a valid JSON object.');
      error.statusCode = 400;
      throw error;
    }
    ...
    if (Effect !== EFFECT_TYPES.ALLOW && Effect !== EFFECT_TYPES.DENY) {
      const error = new Error(`Effect must be exactly '${EFFECT_TYPES.ALLOW}' or '${EFFECT_TYPES.DENY}'.`);
      error.statusCode = 400;
      throw error;
    }
    ...
    if (!Array.isArray(Action) || Action.length === 0) {
      const error = new Error('Action must be a non-empty array of strings.');
      error.statusCode = 400;
      throw error;
    }
    ...
    for (const act of Action) {
      if (!ALL_ACTIONS.includes(act)) {
        const error = new Error(`Action '${act}' is not a valid permission in this system.`);
        error.statusCode = 400;
        throw error;
      }
    }
    ...
    if (!Array.isArray(Resource) || Resource.length !== 1 || Resource[0] !== '*') {
      const error = new Error('Resource must be exactly ["*"].');
      error.statusCode = 400;
      throw error;
    }
  }
 ...
 */
const validateDelegationBypass = (requestingUser, statements) => {
...
const createPolicy = async (requestingUser, { name, type, description, statements, userId, groupId }) => {
  // Validate Type
  if (type !== POLICY_TYPES.MANAGED && type !== POLICY_TYPES.INLINE) {
    const error = new Error(`Type must be either '${POLICY_TYPES.MANAGED}' or '${POLICY_TYPES.INLINE}'.`);
    error.statusCode = 400;
    throw error;
  }
  ...
  if (type === POLICY_TYPES.INLINE) {
    if (!userId && !groupId) {
      const error = new Error('An inline policy must be created for a specific user or group.');
      error.statusCode = 400;
      throw error;
    }
    if (userId && groupId) {
      const error = new Error('An inline policy cannot be created for both a user and a group.');
      error.statusCode = 400;
      throw error;
    }
  }
  ...
  if (existingPolicy) {
    const error = new Error(`Policy with name '${name}' already exists.`);
...
const updatePolicy = async (requestingUser, id, { name, description, statements }) => {
  const existingPolicy = await prisma.policy.findUnique({
    where: { id }
  });
...
const deletePolicy = async (requestingUser, id) => {
  const policy = await prisma.policy.findUnique({
    where: { id },
    include: {
  ...
  if (policy.type === POLICY_TYPES.MANAGED) {
    ...
    if ((attachedUsersCount > 0 || attachedGroupsCount > 0) && !requestingUser.isRoot) {
      const attachedUserNames = policy.users.map(u => u.user.name).join(', ');
      const attachedGroupNames = policy.groups.map(g => g.group.name).join(', ');
      ...
      const error = new Error(`Managed Policy is currently attached and cannot be deleted. Attached to ${attachmentDetails}.`);
      error.statusCode = 400;
      throw error;
    }
  }
